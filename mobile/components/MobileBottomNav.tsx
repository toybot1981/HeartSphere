...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({ currentScreen, onNavigate, hasUnreadMail, onOpenMail }) => {
    ...
    return (
        <div className="fixed bottom-0 left-0 w-full h-16 bg-black/90 backdrop-blur-md border-t border-white/10 flex justify-around items-center z-50 px-2 pb-1 safe-area-bottom">
            <button 
                onClick={() => onNavigate('realWorld')}
                className={`flex flex-col items-center justify-center w-16 h-full transition-colors ${isHome ? 'text-pink-400' : 'text-gray-500 hover:text-white'}`}
            >
            ...
            <button 
                ...
                className={`flex flex-col items-center justify-center w-16 h-full transition-colors ${isWorld ? 'text-purple-400' : 'text-gray-500 hover:text-white'}`}
            >
            ...
            <button 
                ...
                className={`relative -top-5 bg-gradient-to-tr from-indigo-600 to-purple-600 w-14 h-14 rounded-full flex items-center justify-center shadow-lg shadow-purple-500/30 border-2 border-black transition-transform active:scale-95 ${isConnect ? 'ring-2 ring-purple-400' : ''}`}
            >
            ...
            <button 
                onClick={onOpenMail}
                className={`flex flex-col items-center justify-center w-16 h-full transition-colors relative text-gray-500 hover:text-white`}
            >
                {hasUnreadMail && <span className="absolute top-3 right-3 w-2 h-2 bg-red-500 rounded-full animate-pulse" />}
            ...
            <button 
                ...
                className={`flex flex-col items-center justify-center w-16 h-full transition-colors ${isProfile ? 'text-indigo-400' : 'text-gray-500 hover:text-white'}`}
            >
